  ...
  return (
    <header className="relative">
      {/* A wash of the brand tint behind the header, fading into the canvas —
          ...
          hero card. It stays full-bleed while the row inside it is measured by
          the shared content column, so on a monitor the tint still spans the
          window but the greeting lines up with the cards below it. */}
      <div
        aria-hidden="true"
        ...
        <div className="flex shrink-0 items-center gap-2">
          {onExport ? (
            <button
              ...
              onClick={onExport}
              aria-label="Export"
              className="cursor-pointer rounded-full bg-surface p-2.5 text-muted shadow-card transition hover:text-ink hover:brightness-95 active:brightness-95"
            >
              <Download size={19} />
            </button>
          ) : null}
